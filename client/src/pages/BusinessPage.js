...
const BusinessPage = () => {
   ...
   /* if (loading) {
    ...
    const handleSaveVote = async(voteType, businessId) => {
       ...
       const typeMatches = business.votes.filter(userVotes => {
           return userId == userVotes.userId && voteType != userVotes.voteType
       });
                    ...
                    <>
                        ...
                        <Col xs={8} className='mt-2 mb-2  border border-dark border-5 rounded' key={`${data._id}`}>
                            ...
                            <Row>
                                ...
                                <h4>Website:{data.links ? ( <><Button className="btn btn-primary" onClick={e => {e.preventDefault(); window.location = `${data.links[0]}`}}  target="_blank">Website</Button> </>):("")}</h4>
